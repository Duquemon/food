...
public class RestaurantApprovalOutboxScheduler implements OutboxScheduler {
    ...
    public void processOutboxMessage() {
        Optional<List<OrderApprovalOutboxMessage>> status = approvalOutboxHelper
        ...
        if (status.isPresent() && status.get().size() > 0) {
            ...
            log.info("Received {} OrderApprovalOutboxMessage with ids: {}, sending to message",
                    orderApprovalOutboxMessages.size(),
                    orderApprovalOutboxMessages.stream().map(outboxMessage ->
                            outboxMessage.getId().toString()).collect(Collectors.joining(",")));
            orderApprovalOutboxMessages.forEach(om ->
                    restaurantApprovalRequestMessagePublisher.publish(om, this::updateOutboxStatus));

            log.info("{} OrderApprovalOutboxMessage sent to message", orderApprovalOutboxMessages.size());
        }
    }
